...
export class MarketingContextDatasourcesService {
  ...
  async collectAll(organizationId: string, timeRange: MarketingContextTimeRange) {
    ...
    const results = await Promise.all([
      ...
      this.safeCollect('ads.anomalies', () => this.collectAdsAnomalies(organizationId, range)),
      this.safeCollect('email', () => this.collectEmail(organizationId, range)),
      this.safeCollect('zalo', () => this.collectZalo(organizationId, range)),
      ...
      this.safeCollect('businessEvents', () => this.collectBusinessEvents(organizationId, range)),
      this.safeCollect('customer360', () => this.collectCustomer360(organizationId, range)),
      this.safeCollect('customers.repurchase', () => this.collectRepurchase(organizationId, range)),
    ]);
  ...
  async collectCrmLeads(organizationId: string, range: DateRange) {
    ...
    const [total, newCount, hot, unassigned, noFollowUp] = await Promise.all([
      this.prisma.lead.count({ where }),
      this.prisma.lead.count({ where: { ...where, pipelineStatus: 'NEW' } }),
    ...
    return {
      recordCount: total,
      data: { total, new: newCount, hot, unassigned, noFollowUp },
    };
  }
  ...
  async collectEmail(organizationId: string, range: DateRange) {
    const [live, drafts] = await Promise.all([
      ...
      this.prisma.marketingAutopilotEmailDraft.findMany({
        where: {
          organizationId,
          createdAt: { gte: range.from, lte: range.to },
        },
    ...
    const rows = [...live, ...drafts];
    const sent = rows.reduce((s, c) => s + c.sentCount, 0);
    const opens = rows.reduce((s, c) => s + c.openCount, 0);
    const clicks = rows.reduce((s, c) => s + c.clickCount, 0);
    const openRate = sent > 0 ? (opens / sent) * 100 : null;
    const clickRate = sent > 0 ? (clicks / sent) * 100 : null;
    return {
      recordCount: rows.length,
      data: { campaigns: rows.length, sent, openRate, clickRate, liveCampaigns: live.length },
    };
  }
  ...
  async collectBusinessEvents(organizationId: string, range: DateRange) {
    const grouped = await this.prisma.businessEvent.groupBy({
      by: ['eventType'],
      where: {
        organizationId,
    ...
    const counts = Object.fromEntries(grouped.map((g) => [g.eventType, g._count._all]));
    const recordCount = grouped.reduce((s, g) => s + g._count._all, 0);
    return { recordCount, data: { counts, schema: 'business-event.v1' } };
  }

  async collectAdsAnomalies(organizationId: string, range: DateRange) {
    const dateRange = this.dateOnlyRange(range);
    ...
    const rows = grouped.map((g) => {
      ...
      const leads = Number(g._sum.leads ?? 0);
      const clicks = Number(g._sum.clicks ?? 0);
      return {
        adCampaignId: g.adCampaignId,
        spend,
        leads,
        clicks,
    ...
    const withCpl = rows.filter((r) => r.cpl != null).map((r) => r.cpl as number);
    const medianCpl =
      withCpl.length > 0
        ? [...withCpl].sort((a, b) => a - b)[Math.floor(withCpl.length / 2)]!
        : null;
    const anomalous = rows.filter((r) => {
      if (r.spend <= 0) return false;
      if (r.leads === 0 && r.clicks >= 10) return true;
      if (medianCpl != null && r.cpl != null && r.cpl > medianCpl * 3) return true;
      return false;
    });
    return {
      recordCount: rows.length,
      data: {
        anomalousCampaigns: anomalous.length,
        inspectedCampaigns: rows.length,
        wastedSpend: anomalous.reduce((s, r) => s + r.spend, 0),
        medianCpl,
      },
    };
  }

  async collectZalo(organizationId: string, range: DateRange) {
    ...
    return {
      ...
      data: { campaigns: campaigns.length, sent },
    };
  }

  async collectCustomer360(organizationId: string, range: DateRange) {
    const [total, linkedIdentities, events] = await Promise.all([
      this.prisma.customer.count({
        where: { organizationId, isActive: true, mergedIntoId: null },
      }),
      this.prisma.customerIdentity.count({ where: { organizationId } }),
    ...
    const recordCount = total + linkedIdentities + events;
    return {
      recordCount,
      data: { total, linkedIdentities, events },
    };
  }

  async collectRepurchase(organizationId: string, range: DateRange) {
    const now = range.to;
    ...
    const paid = await this.prisma.order.findMany({
      where: {
        organizationId,
        status: { in: ['PAID', 'PARTIALLY_PAID'] },
        orderedAt: { gte: ago90, lte: ago30 },
      },
      select: { customerId: true },
      distinct: ['customerId'],
    });
    const ids = paid.map((p) => p.customerId);
    if (ids.length === 0) {
      return { recordCount: 0, data: { repurchaseCandidates: 0, paidCustomers: 0 } };
    }
    const recentBookings = await this.prisma.appointment.findMany({
      where: {
        organizationId,
        customerId: { in: ids },
        scheduledAt: { gte: ago30 },
        status: { notIn: ['CANCELLED'] },
      },
      select: { customerId: true },
      distinct: ['customerId'],
    });
    const booked = new Set(recentBookings.map((a) => a.customerId).filter(Boolean));
    const repurchaseCandidates = ids.filter((id) => !booked.has(id)).length;
    return {
      recordCount: ids.length,
      data: { repurchaseCandidates, paidCustomers: ids.length },
    };
  }
}
