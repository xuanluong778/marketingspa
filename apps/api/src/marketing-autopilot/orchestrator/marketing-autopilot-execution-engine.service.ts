...
import { ChatbotBotStatus, AutoPostStatus } from '@marketingspa/database';
import { AutoPostFacebookService } from '../../auto-post/auto-post-facebook.service';
import { MarketingAutopilotFacebookPublishService } from './marketing-autopilot-facebook-publish.service';
...
export class MarketingAutopilotExecutionEngineService {
  ...
  constructor(
    ...
    private readonly facebookPublish: MarketingAutopilotFacebookPublishService,
  ) {}
   ...
   * Lifecycle: RECOMMEND → USER APPROVE → RECHECK → EXECUTE → VERIFY → AUDIT
   * Concurrency lock + cooldown + stop-loss + minimum data before any apply.
   */
  async executeApprovedMission(input: {
    ...
    const lockKey = `execute:${missionId}`;
    const locked = await this.acquireExecuteLock(user.organizationId, lockKey, `user:${user.id}`);
    if (!locked) {
      return {
        missionId,
        approvalId,
    ...
    try {
      return await this.executeApprovedMissionLocked(input);
    } finally {
      await this.releaseExecuteLock(user.organizationId, lockKey);
    }
  }

  private async executeApprovedMissionLocked(input: {
    user: AuthUser;
    missionId: string;
    approvalId: string;
  }): Promise<ExecutionEngineResult> {
    ...
    if (guardrail.autopilotMode === 'RECOMMEND_ONLY') {
      return {
        missionId,
        approvalId,
        executed: 0,
        ...
        actions: [
          {
            ...
            action: null,
            result: 'BLOCKED_GUARDRAIL',
            message: 'RECOMMEND_ONLY — không thực thi live',
          },
        ],
      };
    }
    ...
    const lastExec = await this.prisma.marketingAutopilotExecutionLog.findFirst({
      where: { organizationId: user.organizationId, missionId, result: 'EXECUTED' },
      orderBy: { createdAt: 'desc' },
      select: { createdAt: true },
    });
    const observedCpl =
      typeof (snapshot as { kpi?: { cpl?: number } }).kpi?.cpl === 'number'
        ? (snapshot as { kpi: { cpl: number } }).kpi.cpl
        : null;

    const recheck = recheckAutopilotExecute({
      approvalOk: true,
      ...
      okSourceCount: 3,
      observedCpl,
      stopLossCpl: guardrail.stopLossCpl,
      cooldownMs: (guardrail.cooldownMinutes ?? 60) * 60 * 1000,
    });
    ...
    await this.writeLogs({
      ...
      missionId,
      approvalId,
      asset: { module: '', entityType: 'mission', entityId: missionId },
      action: 'RECHECK',
      before: { approvalId },
      gate: {
        ok: recheck.ok,
        stage: recheck.stage,
        result: recheck.ok ? 'PASSED' : 'BLOCKED_GUARDRAIL',
    ...
    for (const asset of mission.assets) {
      ...
      try {
        const execAfter = await this.performExecute(user, action, asset, missionId, approvalId);
        executed += 1;
        results.push({
          module: asset.module,
          entityType: asset.entityType,
          ...
          data: {
            ...
            metadataJson: {
              ...
              lastExecution: {
                ...
                message: execAfter.message,
                at: new Date().toISOString(),
                ...(action === 'PUBLISH_FACEBOOK_CONTENT' && execAfter.after
                  ? {
                      pageId: execAfter.after.pageId,
                      fanpageId: execAfter.after.fanpageId,
                      facebookPostId: execAfter.after.facebookPostId,
                      status: execAfter.after.status,
                      publishedAt: execAfter.after.publishedAt,
                      scheduledAt: execAfter.after.scheduledAt,
                      error: execAfter.after.error,
                      idempotent: execAfter.after.idempotent,
                    }
                  : {}),
              },
            } as Prisma.InputJsonValue,
  ...
  private async checkIntegrationHealth(
    ...
    if (action === 'PUBLISH_FACEBOOK_CONTENT') {
      ...
      if (!post) {
        return { ok: false, reason: 'AutoPost không tồn tại' };
      }
      if (!post.fanpageId) {
        return { ok: false, reason: 'AutoPost chưa gán Fanpage' };
      }
      ...
      if (!page) {
        return { ok: false, reason: 'Fanpage không tồn tại hoặc đã ngắt kết nối' };
      }
      try {
        await this.autoPostFacebook.assertCanPublish(post.userId, organizationId, post.fanpageId);
        return { ok: true };
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
  ...
  private async performExecute(
    ...
    switch (action) {
      ...
      case 'PUBLISH_FACEBOOK_CONTENT': {
        const meta = (asset.metadataJson ?? {}) as { scheduledAt?: string };
        const outcome = await this.facebookPublish.publishApprovedPost({
          user,
          missionId,
          approvalId,
          postId: asset.entityId,
          scheduledAt: typeof meta.scheduledAt === 'string' ? meta.scheduledAt : undefined,
        });
        return {
          message: outcome.idempotent
            ? 'Facebook post idempotent — đã đăng/lên lịch trước đó'
            : outcome.status === AutoPostStatus.SCHEDULED
              ? 'Facebook post scheduled via Auto Post queue'
              : 'Facebook post published via Auto Post',
          after: {
            postId: outcome.postId,
            pageId: outcome.pageId,
            fanpageId: outcome.fanpageId,
      ...
      case 'ENABLE_ADS_CAMPAIGN': {
        const channel = this.channelForAction(action, asset);
        if (channel === 'FACEBOOK' || channel === 'META') {
          return {
            message: 'Facebook Ads enable blocked (read-only — App Review)',
            after: { blocked: true, channel: 'FACEBOOK' },
          };
        }
        const draft = await this.prisma.adDraft.findFirst({
          where: { id: asset.entityId, organizationId: orgId },
        });
        const meta = (asset as { metadataJson?: { externalCampaignId?: string; provider?: string } })
          .metadataJson;
        const externalId = meta?.externalCampaignId;
        if (draft?.platform === 'GOOGLE' || meta?.provider === 'GOOGLE') {
          const googleConn = await this.prisma.adConnection.findFirst({
            where: { organizationId: orgId, provider: 'GOOGLE', status: 'CONNECTED' },
          });
          if (!googleConn) {
            return {
              message: 'Google Ads chưa kết nối — registry only',
              after: { registryOnly: true },
            };
          }
          if (externalId) {
            await this.aiAds.enableCampaign(user, externalId);
            return {
              message: 'Google Ads campaign enabled (live)',
              after: { externalCampaignId: externalId, provider: 'GOOGLE' },
            };
          }
          const googleDraft = await this.prisma.googleAdsCampaignDraft.findFirst({
            where: { id: asset.entityId, organizationId: orgId },
          });
          if (googleDraft) {
            return {
              message: 'Google Ads campaign draft queued for deploy (provider write gated)',
              after: { draftId: googleDraft.id, status: googleDraft.status },
            };
          }
        }
        throw new Error('ENABLE_ADS_CAMPAIGN requires Google campaign id — Meta blocked');
      }
      default:
  ...
  private async acquireExecuteLock(
    organizationId: string,
    lockKey: string,
    holder: string,
    ttlMs = 120_000,
  ...
  private async releaseExecuteLock(organizationId: string, lockKey: string) {
    await this.prisma.marketingAutopilotOutcomeMutationLock.deleteMany({
