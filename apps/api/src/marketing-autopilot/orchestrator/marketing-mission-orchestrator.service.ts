...
import { Prisma, ACTIVATION_EVENT, recordActivationEvent } from '@marketingspa/database';
import {
...
export class MarketingMissionOrchestratorService {
  ...
  async processMission(missionId: string) {
    ...
    if (result.status === 'READY_FOR_APPROVAL') {
      await this.tryAutoApproveFullAutopilot(missionId).catch((err) => {
        this.logger.warn(
          `Auto-approve scan ${missionId}: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
    }
    return result;
  ...
  /** FULL_AUTOPILOT: auto approve+run when guardrails allow (no emergency stop). */
  async tryAutoApproveFullAutopilot(missionId: string) {
    const mission = await this.prisma.marketingMission.findFirst({
      where: { id: missionId, status: 'READY_FOR_APPROVAL' },
    });
    if (!mission) return null;

    const guardrail = await this.guardrailService.getOrCreate(mission.organizationId);
    if (!isFullAutopilotActive(guardrail)) return null;
    ...
    this.logger.log(`[full-autopilot] auto-approve mission ${missionId}`);
    return this.approveAndRun(user, missionId);
  }
  ...
  async approveAndRun(user: AuthUser, missionId: string) {
    ...
    const guardrail = await this.guardrailService.getOrCreate(user.organizationId);
    if (guardrail.emergencyStop) {
      throw new BadRequestException('Emergency Stop đang bật — không thể duyệt/chạy mission');
    }
    if (guardrail.autopilotMode === 'RECOMMEND_ONLY') {
      throw new BadRequestException(
        'RECOMMEND_ONLY — chỉ xem đề xuất, không duyệt/chạy mission',
      );
    }
    ...
    void recordActivationEvent(this.prisma, {
      organizationId: user.organizationId,
      eventType: ACTIVATION_EVENT.FIRST_RECOMMENDATION_ACCEPTED,
      source: 'autopilot.mission.approve',
