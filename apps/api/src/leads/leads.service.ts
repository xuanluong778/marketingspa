...
import { isFunnelPlaceholderLeadName, type FunnelScoringEventType, CUSTOMER_SOURCE } from '@marketingspa/shared';
import {
  CreateLeadDto,
  UpdateLeadDto,
  UpdateLeadStatusDto,
...
export class LeadsService {
  ...
  async findAll(organizationId: string, query: LeadQueryDto) {
    ...
    const cacheName = `list:leads:${page}:${pageSize}:${JSON.stringify({
      s: query.search,
      st: query.pipelineStatus,
      src: query.leadSourceId,
      asg: query.assignedToId,
      br: query.branchId,
    })}`;
  ...
  async create(organizationId: string, dto: CreateLeadDto, userId?: string) {
    ...
    if (duplicate) {
      ...
      const linkedDup = await this.ensureLeadLinkedToCustomer(organizationId, duplicate.id, {
        name: nextName || duplicate.name,
        phone: nextPhone,
        email: nextEmail,
        leadSourceId: dto.leadSourceId || duplicate.leadSourceId,
        branchId: dto.branchId || duplicate.branchId,
        source: this.resolveIngressSource({
          funnelRecommendationId: dto.funnelRecommendationId,
          captureMeta,
          tags: mergedTags,
          platform: attributionPlatform,
      ...
      if (dto.funnelRecommendationId) {
        await this.customerJourney.recordJourneyStep({
          ...
          metadata: {
            ...formSubmitMeta,
            customerId: linkedDup?.customerId,
          },
        });
      }
    ...
    const linked = await this.ensureLeadLinkedToCustomer(organizationId, lead.id, {
      name: lead.name,
      phone: lead.phone,
      email: lead.email,
    ...
    void this.automation.dispatch(organizationId, AutomationTriggerType.LEAD_CREATED, {
      leadId: lead.id,
      customerId: linked?.customerId ?? lead.customerId,
  ...
  private resolveIngressSource(input: {
    funnelRecommendationId?: string | null;
    captureMeta?: Record<string, unknown>;
    tags?: string[] | null;
    platform?: AdPlatform;
  }): string {
    if (input.funnelRecommendationId) return CUSTOMER_SOURCE.FUNNEL_FORM;
    if (input.captureMeta) {
      const channel =
        typeof input.captureMeta.channel === 'string' ? input.captureMeta.channel : '';
      if (channel === 'website' || input.captureMeta.source === 'website_chat') {
        return CUSTOMER_SOURCE.WEBSITE_CHAT;
      }
      return CUSTOMER_SOURCE.WEBSITE_FORM;
    }
    if ((input.tags ?? []).includes('chatbot')) return CUSTOMER_SOURCE.CHATBOT;
    if (input.platform === AdPlatform.META) return CUSTOMER_SOURCE.FACEBOOK_LEAD_ADS;
    return CUSTOMER_SOURCE.WEBSITE_FORM;
  }

  /** Normalize → Customer SSOT → set lead.customerId (idempotent). */
  private async ensureLeadLinkedToCustomer(
    organizationId: string,
    leadId: string,
    input: {
      name: string;
    ...
    const lead = await this.prisma.lead.findFirst({
      ...
      select: { id: true, customerId: true, phone: true, email: true, name: true },
    });
    if (!lead) return null;
    if (lead.customerId) {
      await backfillLeadCustomerId(this.prisma, organizationId, leadId, lead.customerId);
      return { customerId: lead.customerId };
    }
    ...
    try {
      const customer = await this.customers.create(organizationId, {
        name: input.name || lead.name || 'Khách hàng',
        phone: phone || undefined,
        email: email || undefined,
        leadSourceId: input.leadSourceId || undefined,
      ...
      await backfillLeadCustomerId(this.prisma, organizationId, leadId, customer.id);
  ...
  async findStaleLeads(organizationId: string, minutes = 10) {
    const cacheName = `stale:${minutes}`;
    const cached = await this.kpiCache?.getJson<unknown[]>(organizationId, cacheName);
    if (cached) return cached;
    ...
    await this.kpiCache?.setJson(organizationId, cacheName, rows, 15);
    return rows;
  }

  async countByPipeline(organizationId: string): Promise<Record<string, number>> {
    const cached = await this.kpiCache?.getJson<Record<string, number>>(organizationId, 'pipeline');
    if (cached) return cached;
    ...
    for (const row of rows) {
      out[row.pipelineStatus] = row._count._all;
    }
    await this.kpiCache?.setJson(organizationId, 'pipeline', out, 15);
    return out;
  }
