...
import type { FacebookPolicyFinding, FacebookPolicyMediaAnalysis } from './facebook-policy.types';

const MEDIA_CODE = FACEBOOK_ADS_POLICY_CATALOG.MEDIA!.policyCode;
...
export function scanStrongMediaClaims(text: string, prefix: string): FacebookPolicyFinding[] {
  const n = norm(text);
  const findings: FacebookPolicyFinding[] = [];

  const hasWeight = /(giam|giảm)\s*\d+\s*(kg|ký|kilo)/i.test(text) || /giam\s*\d+\s*kg/.test(n);
  const hasTime =
    /(trong|sau)\s*\d+\s*(ngay|ngày|tuan|tuần)/i.test(text) ||
...
export function findingsFromMediaText(text: string, prefix = 'media'): FacebookPolicyFinding[] {
  const t = (text || '').trim();
  if (!t) return [];
  return scanStrongMediaClaims(t, prefix);
}

export function analyzePolicyTranscriptOnly(params: {
  transcript: string;
  caption?: string;
...
export async function analyzePolicyImage(params: {
  buffer: Buffer;
  mimeType: string;
  filename?: string;
  caption?: string;
  openai?: OpenAiService;
}): Promise<FacebookPolicyMediaAnalysis> {
...
export async function analyzePolicyVideo(params: {
  buffer?: Buffer;
  mimeType?: string;
  filename?: string;
  caption?: string;
  ...
  if (!transcript) {
    return {
      ...
      statusHint: 'INSUFFICIENT_DATA',
      message:
        'INSUFFICIENT_DATA: video không có transcript/audio để phân tích — dán transcript hoặc OCR.',
