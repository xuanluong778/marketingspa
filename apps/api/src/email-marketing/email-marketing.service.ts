...
export class EmailMarketingService {
    ...
    const [
      ...
      this.prisma.emailContact.count({
        where: {
          ...
          NOT: { tags: { has: CUSTOMER_360_TEST_TAG || "__c360_e2e__" } },
        },
      }),
  ...
  async updateContact(organizationId: string, id: string, dto: UpdateEmailContactDto) {
    ...
    // CRM Customer is SSOT. EmailContact is a projection — never write back to Customer.
    // Name/phone on a linked contact stay owned by CRM projector.
    if (!current.customerId) {
      if (dto.name !== undefined) data.name = dto.name?.trim() || null;
      if (dto.phone !== undefined) data.phone = dto.phone?.trim() || null;
    } else if (dto.name !== undefined || dto.phone !== undefined) {
      // Ignore CRM-owned fields; consent/tags still apply below.
    }
  ...
  async previewAudience(organizationId: string, listId?: string) {
    ...
    return {
      ...
      capped: contacts.length >= PREVIEW_CAP,
      listId: listId ?? null,
    };
  }

  async listAudienceMembers(organizationId: string, query: AudienceMembersQueryDto) {
    const { page, pageSize, skip, take } = getPaginationParams(query);
    if (query.listId) {
      const list = await this.prisma.emailList.findFirst({
        where: { id: query.listId, organizationId },
      });
      if (!list) throw new NotFoundException('Không tìm thấy nhóm');
    }
    ...
    const suppressedMap = new Map(
      suppressed.map((s) => [s.email.toLowerCase(), s.reason] as const),
    );
    ...
    const eligibleOnly = (query.filter || 'eligible').toLowerCase() !== 'all';

    // Hard cap — never load entire org audience into memory for UI pagination.
    const HARD_CAP = 5000;
    const [totalMatching, contacts] = await Promise.all([
      this.prisma.emailContact.count({ where }),
      this.prisma.emailContact.findMany({
        where,
    ...
    const mapped = contacts.map((c) => {
      ...
      const suppressionReason = suppressedMap.get(email) ?? null;
      let skipReason: string | null = null;
      if (c.status === EmailContactStatus.UNSUBSCRIBED) skipReason = 'unsubscribed';
      else if (c.status === EmailContactStatus.BOUNCED) skipReason = 'hard_bounce';
      else if (suppressionReason === 'COMPLAINT') skipReason = 'complaint';
      else if (suppressionReason === 'BOUNCE') skipReason = 'hard_bounce';
      else if (suppressionReason === 'UNSUBSCRIBE') skipReason = 'unsubscribed';
      else if (suppressionReason) skipReason = 'suppression';
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c.email)) skipReason = 'invalid';
      ...
      const statusLabel =
        skipReason === 'unsubscribed'
          ...
          : skipReason === 'hard_bounce'
            ? 'Hard bounce'
            : skipReason === 'complaint'
              ? 'Complaint'
              : skipReason === 'invalid'
                ? 'Invalid'
                : skipReason === 'suppression'
                  ? 'Suppression'
                  : c.status === EmailContactStatus.SUBSCRIBED
                    ? 'Nhận email'
                    : c.status;
      ...
      return {
        ...
        email: c.email,
        source: c.source,
        groups: c.listMembers.map((m) => m.list.name),
        status: c.status,
        statusLabel,
        eligible: !skipReason,
        skipReason,
    ...
    const filtered = eligibleOnly ? mapped.filter((m) => m.eligible) : mapped;
    const total = eligibleOnly ? filtered.length : Math.min(totalMatching, HARD_CAP);
  ...
  async syncFromCrm(organizationId: string) {
    ...
    const processChunk = async (rows: AudienceUpsert[]) => {
      for (const row of rows) {
        const result = await this.upsertAudienceContact(organizationId, row);
        if (!result) {
          skipped += 1;
          continue;
        }
  ...
  private async upsertAudienceContact(
    ...
    // Keep E2E / test tags out of live audience mutations unless already tagged.
    if (isCustomer360TestTag(row.tags) && !row.customerId) {
      // still allow explicit test runs that pass customerId
    }
      ...
      try {
        const resolved = await this.prisma.$transaction(async (tx) =>
          resolveCustomerForIngress(tx, {
            ...
            email,
            phone: row.phone,
            source: row.source?.trim() || CUSTOMER_SOURCE.EMAIL_MARKETING,
            tags: parseTagList(row.tags),
          }),
        );
        if (resolved.customer) customerId = resolved.customer.id;
      } catch {
        customerId = null;
      }
    }
  ...
  async createCampaign(organizationId: string, dto: CreateEmailCampaignDto, userId?: string) {
    ...
    const contactIds = await this.normalizeContactIds(organizationId, dto.contactIds);
    ...
    return this.prisma.emailCampaign.update({
      ...
      data: {
        ...
        ...(dto.contactIds !== undefined
          ? { selectedContactIds: await this.normalizeContactIds(organizationId, dto.contactIds) }
          : {}),
      },
    });
  }

  private async normalizeContactIds(organizationId: string, contactIds?: string[]) {
    if (!contactIds?.length) return [] as string[];
    const unique = [...new Set(contactIds.map((id) => id.trim()).filter(Boolean))];
    if (unique.length > 20_000) {
      throw new BadRequestException('Quá nhiều người nhận được chọn');
    }
    ...
    if (rows.length !== unique.length) {
      throw new BadRequestException('Một số contact không thuộc tổ chức này');
    }
...
- footer 1-2 dòng, không spam.`,
  ...
  async sendTestEmail(dto: SendTestEmailDto) {
    ...
    if (!provider.isConfigured()) {
      if (health.marketing === 'ses' || health.resolved === 'ses') {
        throw new BadRequestException(sesNotConfiguredMessage());
      }
      if (health.marketing === 'brevo') {
        throw new BadRequestException('Chưa cấu hình Brevo (BREVO_API_KEY, BREVO_FROM_EMAIL).');
      }
