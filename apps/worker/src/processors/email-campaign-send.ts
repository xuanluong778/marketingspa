...
function fromAddress(fromName?: string | null, fromEmail?: string | null): string {
  ...
  if (resolveEmailProviderName('marketing') === 'ses') return sesFromEmail();
  if (resolveEmailProviderName('marketing') === 'brevo') {
    return (process.env.BREVO_FROM_EMAIL || process.env.SES_FROM_EMAIL || process.env.SMTP_FROM || '').trim();
  }
...
export async function processEmailCampaignSend(job: Job<EmailSendJob>, redis: Redis) {
  ...
  if (!template) {
    await prisma.emailCampaignRecipient.updateMany({
      ...
      data: { status: EmailRecipientStatus.FAILED, lastError: 'missing_template', failedAt: new Date() },
    });
  ...
  for (const recipientId of recipientIds) {
    ...
    const claimed = await prisma.emailCampaignRecipient.updateMany({
      ...
      data: {
        ...
        idempotencyKey: recipient.idempotencyKey || recipient.id,
      },
    });
    if (claimed.count === 0) {
      skipped += 1;
...
async function failRecipient(
  ...
  await prisma.$transaction([
    prisma.emailCampaignRecipient.update({
      where: { id: recipientId },
      data: { status: EmailRecipientStatus.FAILED, lastError: lastError.slice(0, 500), failedAt: new Date() },
    }),
    prisma.emailEvent.create({
      data: {
        organizationId,
