...
import type {
  ...
  MarketingAutopilotCommandCenter,
...
export function useMarketingAutopilotCommandCenter(enabled = true) {
  return useQuery({
    queryKey: ['marketing-autopilot', 'command-center'],
    queryFn: () =>
      apiClient<MarketingAutopilotCommandCenter>('/marketing-autopilot/command-center'),
    enabled,
    retry: false,
    staleTime: 30_000,
  });
...
export function useMarketingAutopilotEmergencyStop() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (emergencyStop: boolean) =>
      apiClient<{ emergencyStop: boolean }>('/marketing-autopilot/emergency-stop', {
        method: 'POST',
        body: JSON.stringify({ emergencyStop }),
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['marketing-autopilot', 'command-center'] });
    },
  });
}
