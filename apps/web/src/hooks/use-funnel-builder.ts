  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient<
        | FunnelStageRow[]
        | { id: string; name: string; stages?: FunnelStageRow[] }
      >('/crm/pipeline');
...
export function useFunnelTemplate(idOrSlug: string | null) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient<FunnelTemplateDetail>(`/funnel-builder/templates/${idOrSlug}`),
    enabled: !!idOrSlug,
  });
}
...
export function useFunnelScoring(funnelId: string | null) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient<FunnelScoringConfig | null>(
        `/funnel-builder/recommendations/${funnelId}/scoring`,
      ),
  });
}
...
export function useFunnelRecommendation(id: string | null) {
  return useQuery({
    ...
    enabled: Boolean(id),
    queryFn: () =>
      apiClient<FunnelGeneratorResult & {
...
export function useFunnelBlueprints(status?: string) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiClient<FunnelBlueprintListItem[]>(
        `/funnel-builder/blueprints${qs ? `?${qs}` : ''}`,
      ),
  });
}
