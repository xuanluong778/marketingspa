...
function TaskList({ items }: { items: BucketTask[] }) {
  if (!items.length) {
    return <p className="text-sm text-muted-foreground py-2">Không có công việc.</p>;
  }
  return (
    <ul className="space-y-2">
      {items.map((t) => (
        <li key={t.id} className="rounded-lg border p-2.5 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="font-medium">{t.title}</p>
              <p className="text-xs text-muted-foreground">
                {t.project?.name || '—'} · {t.columnName}
            ...
            <div className="flex gap-1">
              ...
              {t.risk && t.risk !== 'NONE' && (
                <Badge variant={t.risk === 'OVERDUE' ? 'destructive' : 'secondary'}>
                  {t.risk === 'OVERDUE' ? 'Quá hạn' : 'Nguy cơ chậm'}
                </Badge>
              )}
            </div>
          </div>
...
export default function MyWorkPage() {
  ...
  if (meQ.isLoading || (meQ.isSuccess && q.isLoading)) {
    return <LoadingState message="Đang tải việc của tôi…" />;
  }
  if (meQ.isError) {
    const msg =
  ...
  if (!q.data) return <EmptyState title="Không có dữ liệu" />;

  const sections: Array<{ key: string; title: string }> = [
    { key: 'today', title: 'Hôm nay' },
    { key: 'upcoming', title: 'Sắp đến hạn' },
    { key: 'overdue', title: 'Quá hạn' },
    { key: 'inProgress', title: 'Đang làm' },
    { key: 'pendingReview', title: 'Chờ duyệt' },
    { key: 'done', title: 'Hoàn thành' },
    { key: 'assignedByMe', title: 'Việc tôi giao' },
  ];
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Việc của tôi"
        description={`Timezone ${q.data.timezone} · Ngày ${q.data.today}`}
      />
      <SubNav />
      {q.data.warning ? (
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Báo cáo theo dự án / phòng ban / nhân viên · không chỉ đếm số việc hoàn thành"
      />
      <SubNav />
...
export function WorkCalendarPageInner() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Lịch công việc" description="Xem theo tuần / tháng (deadline)" />
      <SubNav />
