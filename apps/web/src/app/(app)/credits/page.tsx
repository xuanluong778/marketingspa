...
export default function CreditsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AI Credit"
        description="Credit dùng cho các tác vụ AI. Hết Credit không khóa CRM, Lead, Funnel, HRM hay Inbox."
      />
      ...
      <div>
        <h2 className="mb-3 text-base font-semibold">Mua thêm Credit</h2>
        ...
        {packages.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">Chưa có gói Credit.</p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-3">
      ...
      <div>
        <h2 className="mb-3 text-base font-semibold">Lịch sử giao dịch</h2>
        {history.isLoading ? (
          <LoadingState />
        ) : history.isError ? (
          <ErrorState onRetry={() => void history.refetch()} />
        ) : !history.data?.items.length ? (
          <p className="text-sm text-muted-foreground">Chưa có giao dịch Credit.</p>
        ) : (
      ...
      <DialogContent className="max-w-md">
        ...
        {isLoading || !order ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground">
            <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Đang tải đơn...
          </div>
        ) : (
