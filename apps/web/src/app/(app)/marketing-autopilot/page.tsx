...
import { Bot, Lock, Sparkles, BarChart3, History, Gauge } from 'lucide-react';
import {
  ...
  useMarketingAutopilotCommandCenter,
...
import { AutopilotCommandCenter } from '@/components/marketing-autopilot/autopilot-command-center';
...
export default function MarketingAutopilotPage() {
  ...
  const activeTab: AutopilotPageTab = pageQuery.tab ?? 'command';
  ...
  const commandCenter = useMarketingAutopilotCommandCenter(enabled && activeTab === 'command');
  ...
  return (
    ...
    >
      ...
      >
        <TabsList
          className={cn(
            'grid h-auto w-full grid-cols-1 gap-1 p-1 sm:grid-cols-4',
            'bg-muted',
            isCreateTab && 'h-9 shrink-0 sm:grid-cols-4',
          )}
        >
          <TabsTrigger
            value="command"
