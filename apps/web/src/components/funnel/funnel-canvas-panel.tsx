...
function FunnelDesignLoading() {
  const t = useT();
  return <LoadingState message={t('funnel.loadingDesign')} />;
}

function FunnelDraftLoading() {
  const t = useT();
  return <LoadingState message={t('funnel.loadingDraft')} />;
}
...
export function FunnelCanvasPanel({
  ...
  return (
    <Suspense fallback={<FunnelDesignLoading />}>
...
function FunnelCanvasPanelInner({
  ...
  if (!embedded && list.isLoading) return <FunnelDraftLoading />;
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {draftsWithComplete.length === 0 ? (
            <EmptyState
              title={t('funnel.noFunnelToDesign')}
              description={t('funnel.goCreateHint')}
            />
          ) : (
