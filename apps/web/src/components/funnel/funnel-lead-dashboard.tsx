...
export function FunnelLeadDashboard({ leadId, funnelId, compact, onBack, onEdit }: Props) {
  ...
  if (leadQuery.isLoading) return <LoadingState message={t('funnel.loadingLeads')} />;
      ...
      <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {[lead.phone, lead.email].filter(Boolean).join(' · ') || t('funnel.noPhoneEmail')}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {resolvedFunnelId ? (
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={() => router.replace(funnelHref({ tab: 'mine', design: resolvedFunnelId }))}
            >
              Xem phễu
            </Button>
          ) : null}
      ...
      <div className={cn('grid gap-3', compact ? 'grid-cols-1' : 'md:grid-cols-2 xl:grid-cols-3')}>
        ...
        <SectionCard title="Thông tin form">
          {form.rows.length ? (
            ...
            <EmptyHint text={t('funnel.noFormAnswers')} />
          )}
          {form.formId ? (
        ...
        <SectionCard title="Booking">
          {bookings.length === 0 ? (
            <EmptyHint text={t('funnel.noAppointments')} />
          ) : (
        ...
        <SectionCard title="Purchase">
          {orders.length === 0 ? (
            <EmptyHint text={t('funnel.noOrders')} />
          ) : (
            <ul className="space-y-2">
