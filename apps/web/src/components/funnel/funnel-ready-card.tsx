    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4 text-sm">
          {spec.cta && (
            <p>
              <span className="text-muted-foreground">Nút kêu gọi: </span>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            <Button
              variant="outline"
              onClick={() => router.replace(funnelHref({ tab: 'mine', draft: recommendationId }))}
            >
              Chỉnh sửa
            </Button>
          </div>
          ...
          {status !== 'ACTIVE' ? (
            <p className="text-xs text-muted-foreground">
              Link khách chỉ nhận lead sau khi kích hoạt: {publicUrl}
            </p>
          ) : null}
        </CardContent>
      </Card>
