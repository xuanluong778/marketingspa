    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={openNew}>
          <Plus className="mr-2 h-4 w-4" /> Tạo mẫu
        </Button>
      </div>
      <DataTable
        getRowKey={(r) => r.id}
        data={(templates.data?.items ?? []).filter((t) => t.category !== 'campaign')}
        isLoading={templates.isLoading}
        isError={templates.isError}
        onRetry={() => templates.refetch()}
        emptyTitle="Chưa có mẫu email"
        columns={[
      ...
      <Dialog open={!!form} onOpenChange={(o) => !o && setForm(null)}>
        <DialogContent className="flex max-h-[92vh] max-w-5xl flex-col gap-4 overflow-hidden">
          <DialogHeader>
            <DialogTitle>{editing ? 'Sửa mẫu email' : 'Tạo mẫu email'}</DialogTitle>
          </DialogHeader>
          {form && (
            <form
