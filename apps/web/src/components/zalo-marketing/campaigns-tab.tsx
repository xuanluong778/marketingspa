...
export function ZaloCampaignsTab() {
  ...
  if (campaigns.isLoading) return <LoadingState label="Đang tải chiến dịch Zalo..." />;
  if (campaigns.isError) return <ErrorState message="Không tải được chiến dịch" onRetry={() => campaigns.refetch()} />;

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Tạo chiến dịch ZBS hoặc Broadcast theo OA — queue BullMQ, chống trùng, tôn trọng quota.
        </p>
      ...
      {items.length === 0 ? (
        <EmptyState
          title="Chưa có chiến dịch Zalo"
          description="Bắt đầu bằng cách kết nối OA và tạo chiến dịch ZBS/Broadcast."
          action={
            <Button onClick={() => setWizardOpen(true)}>
              <Plus className="mr-2 h-4 w-4" />
              Tạo chiến dịch
