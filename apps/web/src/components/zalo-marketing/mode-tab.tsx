...
export function ZaloModeTab({ mode }: Props) {
  ...
  if (campaigns.isLoading) return <LoadingState label={`Đang tải ${meta.title}...`} />;
  if (campaigns.isError) {
    return <ErrorState message="Không tải được chiến dịch" onRetry={() => campaigns.refetch()} />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <EmptyState
          title={`Chưa có chiến dịch ${meta.title}`}
