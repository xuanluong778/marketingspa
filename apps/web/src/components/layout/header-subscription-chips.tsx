...
import {
  localizedPlanLabel,
  localizedUpgradeLabel,
} from '@/lib/subscription-display-labels';
...
export function HeaderSubscriptionChips() {
  const t = useT();
  const { display, data: subscriptionData } = useSubscriptionDisplay();
  if (!display) return null;

  const trialDays = subscriptionData?.trial?.trialDays ?? 3;
  const planLabel = localizedPlanLabel(t, display.tier, trialDays);
  const days = display.remainingDays;
  const upgrade = display.upgradeButton;
  const upgradeLabel = upgrade ? localizedUpgradeLabel(t, upgrade.variant) : '';
  ...
  return (
    <>
      ...
      {showCurrentPlanBadge ? (
        <Link href="/pricing" title={planLabel} className={planBadgeClass}>
