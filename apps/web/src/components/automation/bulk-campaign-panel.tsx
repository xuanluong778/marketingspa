...
export function BulkCampaignPanel({ prefill, onPrefillConsumed }: Props) {
  ...
  useEffect(() => {
    ...
    setEditingId(null);
    setForm({
      name: prefill.name || t('automation.bulkDefaultNameNew'),
  ...
  const templateItems = (templates.data?.items ?? []).filter((row) => row.channel === form.channel);

  function resetForm() {
    setEditingId(null);
    setForm({
      name: t('automation.bulkDefaultNameNew'),
      channel: 'MESSENGER',
      channelConnectionId: '',
      messageTemplateId: '',
      body: '',
  ...
  function openEdit(c: MessagingCampaignDetail) {
    ...
    setEditingId(c.id);
    setForm({
      name: c.name || t('automation.bulkDefaultName'),
      channel: c.channel,
  ...
  async function submitForm() {
    ...
    try {
      ...
      const payload = {
        name: form.name.trim() || t('automation.bulkDefaultName'),
  ...
  return (
    <div className="space-y-4">
      ...
      {campaigns.isLoading && <LoadingState message={t('automation.loadingCampaigns')} />}
      {campaigns.isError && <ErrorState onRetry={campaigns.refetch} />}
      {!campaigns.isLoading && !campaigns.isError && items.length === 0 && (
        <EmptyState
          title={t('automation.emptyCampaigns')}
          description={t('automation.emptyCampaignsHint')}
        />
      )}
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingId ? t('automation.editBulk') : t('automation.createBulk')}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              <Label>{t('automation.campaignName')}</Label>
              <Input
                value={form.name}
            ...
            <div className="space-y-2 rounded-md border p-3">
              ...
              {form.channelConnectionId && identities.isLoading && (
                <p className="text-xs text-muted-foreground">{t('automation.loadingContacts')}</p>
              )}
              {form.channelConnectionId && !identities.isLoading && contactItems.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  {t('automation.noNicksOnPage')}
                </p>
              )}
          ...
          <DialogFooter>
            ...
            >
              {submitting
                ...
                : editingId
                  ? form.sendMode === 'schedule'
                    ? t('automation.saveAndSchedule')
                    : t('common.save')
                  : form.sendMode === 'schedule'
                    ? t('automation.createAndSchedule')
                    : t('automation.createAndSend')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
