...
export type AutopilotPageTab = 'create' | 'analysis' | 'history';
...
const TAB_VALUES = new Set(['create', 'analysis', 'history']);
...
export function parseAutopilotPageQuery(
  ...
  const tabRaw = searchParams.get('tab') ?? 'create';
  ...
  return {
    ...list,
    tab: TAB_VALUES.has(tabRaw) ? (tabRaw as AutopilotPageTab) : 'create',
...
export function buildAutopilotPageQueryString(query: MarketingAutopilotPageQuery): string {
  ...
  set('tab', query.tab && query.tab !== 'create' ? query.tab : undefined);
  set(
    'view',
