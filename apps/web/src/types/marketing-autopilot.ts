...
export interface CommandCenterNba {
  priority: number;
  title: string;
  whyNow: string;
  evidence: string;
...
export interface MarketingAutopilotCommandCenter {
  version: string;
  snapshotId: string | null;
  fromCache: boolean;
  generatedAt: string;
  ...
  nextBestActions: CommandCenterNba[];
  sources: Array<{ domain: string; status: string; recordCount?: number; error?: string }>;
  coverage: {
    required: string[];
    present: string[];
  ...
  safety: {
    ...
    llmInventedMetrics: boolean;
    facebookReadOnly?: boolean;
  };
  operational?: {
    autopilotMode: string;
    effectiveMode: string;
    fullAutopilotEnabled: boolean;
    fullAutopilotActive: boolean;
    emergencyStop: boolean;
    cooldownMinutes: number;
    pendingMissionApprovals: number;
    runningMissions: number;
    pendingOptimizationApprovals: number;
