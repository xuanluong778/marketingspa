...
// Schedule datetime input opens native picker on click (Auto Post action bar only).
const scheduleInputSrc = readFileSync(
  new URL('../apps/web/src/components/content-auto-post/auto-post-schedule-datetime-input.tsx', import.meta.url),
  'utf8',
);
const libraryPanel = readFileSync(
  new URL('../apps/web/src/components/content-auto-post/auto-post-from-library-panel.tsx', import.meta.url),
  'utf8',
);
assert(scheduleInputSrc.includes("type=\"datetime-local\""), 'schedule input type');
assert(scheduleInputSrc.includes('onClick={openPicker}'), 'schedule input opens picker on click');
assert(scheduleInputSrc.includes('showPicker'), 'schedule input showPicker');
assert(libraryPanel.includes('AutoPostScheduleDatetimeInput'), 'library panel uses schedule input');
