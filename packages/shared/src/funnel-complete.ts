...
export const funnelAppearanceSchema = z
  .object({
    primaryColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    backgroundColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    headingColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    ctaBackgroundColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    ctaTextColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
  })
  .strict();

export type FunnelAppearance = z.infer<typeof funnelAppearanceSchema>;
...
export const funnelCompleteSpecSchema = z
  .object({
    ...
    /** Optional public page theme — inline editor only */
    appearance: funnelAppearanceSchema.optional(),
...
function defaultLeadForm(service: string) {
  return {
    ...
    fields: [
      ...
      { key: 'preferred_time', label: 'Khung giờ thuận tiện', type: 'datetime' as const, required: false, placeholder: 'Chọn ngày và giờ' },
      {
        key: 'contact_via',
        label: 'Kênh liên hệ',
        type: 'contact_channel' as const,
        required: false,
        options: ['Zalo', 'Điện thoại', 'Email', 'Messenger', 'WhatsApp', 'SMS', 'Khác'],
      },
