...
export const marketingAutopilotGuardrailSchema = z.object({
  ...
  autopilotMode: z
    .enum(['RECOMMEND_ONLY', 'APPROVAL_AUTOPILOT', 'FULL_AUTOPILOT'])
    .default('APPROVAL_AUTOPILOT'),
  fullAutopilotEnabled: z.boolean().default(false),
  emergencyStop: z.boolean().default(false),
  cooldownMinutes: z.number().int().min(0).max(24 * 60).default(60),
});
...
export type MarketingAutopilotMode =
  | 'RECOMMEND_ONLY'
  | 'APPROVAL_AUTOPILOT'
  | 'FULL_AUTOPILOT';

export type AutopilotMode = MarketingAutopilotMode;

export function normalizeMarketingAutopilotMode(raw: string | null | undefined): MarketingAutopilotMode {
  const v = String(raw ?? 'APPROVAL_AUTOPILOT').toUpperCase();
  if (v === 'RECOMMEND_ONLY') return 'RECOMMEND_ONLY';
  if (v === 'FULL_AUTOPILOT') return 'FULL_AUTOPILOT';
  return 'APPROVAL_AUTOPILOT';
}

/** FULL autopilot requires explicit opt-in + mode + no emergency stop. */
export function isFullAutopilotActive(input: {
  ...
  return (
    !input.emergencyStop &&
    input.fullAutopilotEnabled &&
    normalizeMarketingAutopilotMode(input.autopilotMode) === 'FULL_AUTOPILOT'
  );
}
...
export function evaluateExecutionGate(input: ExecutionGateInput): ExecutionGateOutcome {
  ...
  if (action === 'PUBLISH_FACEBOOK_CONTENT' && !g.allowFacebookPublish) {
    return {
      ...
      message: 'allowFacebookPublish=false — bật trong Autopilot guardrail để đăng Fanpage',
    };
  }
...
export type AutopilotFacebookPublishBlockReason =
  | 'emergency_stop'
  | 'recommend_only'
  | 'allow_facebook_publish_off';

/** Central guard for Autopilot → Fanpage publish (opt-in only; never implied by FULL_AUTOPILOT). */
export function getAutopilotFacebookPublishBlock(guardrail: MarketingAutopilotGuardrailConfig):
  | { blocked: true; reason: AutopilotFacebookPublishBlockReason; message: string }
  | { blocked: false } {
  ...
  if (normalizeMarketingAutopilotMode(guardrail.autopilotMode) === 'RECOMMEND_ONLY') {
    return {
      blocked: true,
      reason: 'recommend_only',
      message: 'RECOMMEND_ONLY — không đăng Fanpage',
    };
  }
  if (!guardrail.allowFacebookPublish) {
    return {
      blocked: true,
      reason: 'allow_facebook_publish_off',
      message: 'allowFacebookPublish=false — bật opt-in riêng trong guardrail',
    };
  }
...
export function normalizeGuardrailFromDb(row: {
  ...
  return {
    ...
    autopilotMode: normalizeMarketingAutopilotMode(row.autopilotMode),
    fullAutopilotEnabled: row.fullAutopilotEnabled ?? false,
    emergencyStop: row.emergencyStop ?? false,
    cooldownMinutes: row.cooldownMinutes ?? DEFAULT_MARKETING_AUTOPILOT_GUARDRAIL.cooldownMinutes,
  };
}
