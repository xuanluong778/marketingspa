...
import {
  ...
  FUNNEL_LEGACY_TIME_SLOTS,
  isFunnelContactChannelPreset,
  isFunnelDatetimeValue,
  isValidVietnamesePhone,
  mapFunnelContactChannelToCode,
  parseFunnelDatetimeLocal,
} from './funnel-form-fields';
...
export function formatFunnelFieldDisplayValue(
  field: Pick<LeadFormField, 'key' | 'type' | 'options'>,
  raw: string,
): string {
  if (!raw) return '';
  if (
    field.type === 'datetime' ||
    (field.key === 'preferred_time' && isFunnelDatetimeValue(raw))
  ) {
    return formatFunnelDatetimeDisplay(raw);
  }
  return raw;
}
...
export function validateFunnelFormAnswers(
  ...
  for (const field of fields) {
    ...
    if ((field.type === 'tel' || isPhoneField(field)) && !isValidVietnamesePhone(text)) {
      issues.push(`${field.label} không hợp lệ (SĐT Việt Nam 10 số, bắt đầu 0)`);
    }
    if (
      ...
      if (!(field.key === 'preferred_time' && isFunnelDatetimeValue(text))) {
    ...
    if (field.type === 'datetime' || field.key === 'preferred_time') {
      if (
        text &&
        !isFunnelDatetimeValue(text) &&
        !(field.key === 'preferred_time' && (FUNNEL_LEGACY_TIME_SLOTS as readonly string[]).includes(text))
      ) {
        issues.push(`${field.label}: chọn ngày và giờ hợp lệ`);
      }
    }
    if (field.type === 'contact_channel' || field.key === 'contact_via') {
      const presets =
        field.type === 'contact_channel'
          ...
          : field.options?.length
            ? field.options
            : [...FUNNEL_CONTACT_CHANNEL_OPTIONS];
      if (field.type === 'contact_channel' && text === FUNNEL_CONTACT_CHANNEL_OTHER) {
        issues.push(`Vui lòng nhập ${field.label} khi chọn Khác`);
      } else if (text.length < 2) {
        issues.push(`${field.label} không hợp lệ`);
      } else if (
        field.type === 'contact_channel' &&
        !isFunnelContactChannelPreset(text, presets) &&
        text.length > 80
      ) {
        issues.push(`${field.label} quá dài`);
      } else if (
        field.type === 'radio' &&
        field.key === 'contact_via' &&
        field.options?.length &&
        !field.options.includes(text)
      ) {
        issues.push(`${field.label}: giá trị không nằm trong lựa chọn`);
...
export function mapFunnelFormAnswersToLead(
  ...
  for (const field of spec.leadForm.fields) {
    ...
    extra.push({
      key: field.key,
      label: field.label,
      value: formatFunnelFieldDisplayValue(field, value),
    });
    if (field.key === 'contact_via' || field.type === 'contact_channel') {
      extra.push({
        key: 'contact_via_code',
        label: 'Mã kênh liên hệ',
        value: mapFunnelContactChannelToCode(value),
      });
    }
    if (field.key === 'preferred_time' && isFunnelDatetimeValue(value)) {
      const { date, time } = parseFunnelDatetimeLocal(value);
      if (date) extra.push({ key: 'preferred_date', label: 'Ngày hẹn', value: date });
      if (time) extra.push({ key: 'preferred_time_slot', label: 'Giờ hẹn', value: time });
    }
  }
